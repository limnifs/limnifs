...
pub fn max_ratio() -> WriteConfig {
    WriteConfig {
        defaults: Defaults {
            ...
            shared_inline: true,
            // Ratio over read-cost: keep whole-file categorizer
            // claims at any size (0 = unlimited) and monolithic (a
            // container's independent frames cost 1-3%).
            max_drop_size: 0,
            seekable_drops: false,
...
pub fn max_speed() -> WriteConfig {
    WriteConfig {
        defaults: Defaults {
            ...
            metadata_externalize_threshold: crate::METADATA_EXTERNALIZE_THRESHOLD,
            shared_inline: true,
            max_drop_size: crate::config::DEFAULT_MAX_DROP_SIZE,
            seekable_drops: true,
